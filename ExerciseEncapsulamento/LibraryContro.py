...
class Biblioteca:
    ...
    def SearchingBookName(self, name):
        print('Searching...')
        for livro in self.livros:
            if name.upper() in livro.titulo.upper():
                print(f'Livros na biblioteca "{livro.titulo}": - ISBN: {livro.isbn}')
                return livro
        print(f'O Livro nao "{name}" foi encontrado na biblioteca')
        return None
...
# Buscando o livro pelo nome
biblioteca.SearchingBookName('campo')